...
export default async function VideosPage({ params }: PageProps) {
  ...
  if (!token) redirect(`/login?redirect=/lms/course/${slug}/phase/${phaseId}/week/${weekId}/videos`);

  const userData = cookieStore.get("user")?.value;
  if (!userData) redirect(`/login?redirect=/lms/course/${slug}/phase/${phaseId}/week/${weekId}/videos`);

  let user: any;
  try { user = JSON.parse(userData!); } catch { redirect(`/login?redirect=/lms/course/${slug}/phase/${phaseId}/week/${weekId}/videos`); }
  ...
  return (
    <LMSLayout>
      <WeekDetailView course={course} phase={phase} week={week} activeTab="videos" />
    </LMSLayout>
  );
}
